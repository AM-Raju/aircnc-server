...
const CheckoutForm = ({ closeModal, bookingInfo }) => {
  ...
  useEffect(() => {
    ...
    if (bookingInfo?.price) {
      axiosSecure.post("/create-payment-intent", { price: bookingInfo?.price }).then((res) => {
        console.log("Dhaka", res.data.clientSecret);
  ...
  const handleSubmit = async (event) => {
    ...
    // Error handling of create payment method
    if (error) {
    ...
    // Error handling of confirm payment intent
    if (confirmError) {
    ...
    <>
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex mt-2 justify-around">
          ...
          >
            {processing ? (
              <ImSpinner className="text-2xl animate-spin"></ImSpinner>
            ) : (
